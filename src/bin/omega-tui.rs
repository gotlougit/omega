...
use picrust::omega_loop_client::{
    AgentdClient, DaemonReader, DaemonWriter, OutputChunk, ServerEvent, SessionConfig,
};
use picrust::tui::{Color, Event, Span, Style, StyledBlock, StyledText, Term, TermHandle};
...
struct Streaming {
    block_id: Option<picrust::tui::BlockId>,
    buf: String,
...
fn tool_call_line(name: &str, input: &serde_json::Value) -> String {
    ...
    for key in PRIMARY_KEYS {
        if let Some(value) = input.get(*key).and_then(|v| v.as_str()) {
            ...
            if !one_line.is_empty() {
                return format!("tool call {name}: {}", picrust::tui::truncate_to_width(&one_line, 120));
            }
        }
    }
    ...
    if json == "{}" || json == "null" {
        ...
        format!("tool call {name}: {}", picrust::tui::truncate_to_width(&json, 120))
    }
}
        ...
        ServerEvent::Chunk { chunk, .. } => match chunk {
            ...
            OutputChunk::ToolEnd { result, .. } => {
                if result.is_error {
                    ...
                    let preview = picrust::tui::truncate_to_width(&result.text, 80);
                    handle.print_output(StyledBlock::new(StyledText::from(Span::new(
                        format!("  ✓ {preview}"),
                        s_tool(),
                    ))));
...
mod tests {
    ...
    use picrust::omega_loop_client::{OutputChunk, ServerEvent, ToolResultWire};
    use picrust::tui::emulator::{Capture, Emulator};
    use picrust::tui::{BlockId, RawEvent};
    ...
    fn toolend_ok_empty_text_is_silent() {
        ...
        handle_daemon_event(&fx.handle, &mut fx.app, &mut fx.streaming,
            chunk(OutputChunk::ToolEnd {
                id: "t1".into(),
                result: picrust::omega_loop_client::ToolResultWire {
                    text: String::new(),
                    is_error: false,
                    content: None,
                },
